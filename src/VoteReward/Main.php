...
class Main extends PluginBase {
      ...
      case "vote":
        if(isset($args[0]) && strtolower($args[0]) == "reload") {
          if(Utils::hasPermission($sender, "votereward.command.reload")) {
            $this->reload();
            $sender->sendMessage("§a[VoteReward] All configurations have been reloaded.");
            break;
          }
        ...
        if(in_array(strtolower($sender->getName()), $this->queue)) {
          $sender->sendMessage("§a[VoteReward] §bSlow down! We're already checking lists for you.");
          break;
        }
        ...
        break;
      default:
        $sender->sendMessage("§a[VoteReward] §cInvalid Command.");
        break;
    }
    return true;
  }
  ...
  public function rewardPlayer($player, $multiplier) {
    ...
    if($multiplier < 1) {
      $player->sendMessage("§a[VoteReward]§e You haven't voted on any server lists!");
      return;
    }
    $clones = [];
    ...
    $player->sendMessage("§a[VoteReward]§b You voted on $multiplier server list" . ($multiplier == 1 ? "" : "s") . "!");
  }
